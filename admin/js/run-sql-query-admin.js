(function( $ ) {
	...
	function run_sql_query_2() {
		...
		var query =" SELECT ID, post_content, post_title FROM wp_posts WHERE ID =1 ";
	...
	function run_sql_query_3() {
		...
		var query =" SELECT *, user_email, user_nicename FROM wp_users WHERE user_registered BETWEEN 'dateControl.value' AND 'dateEnd.value'";
	...
	$(function() {
		...
		$('#run_posts_query_button').on('click', function() {
			if(check_sql_query()) {
				run_sql_query_2();
			}
		});
		$('#run_users_query_button').on('click', function() {
			if(check_sql_query()) {
				run_sql_query_3();
			}
		});
